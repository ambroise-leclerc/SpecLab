...
- `speclab_options` and `speclab_warnings` are linked **privately** (and wrapped in
  ...
  links `speclab::speclab`. Only link options (e.g. `-fsanitize=`) still propagate,
  through `$<LINK_ONLY:>`, which a sanitizer build needs. Keep it that way: a
  `PUBLIC` link here once injected `MDUX_VERSION_*=0.1.0` into every MduX test TU.
